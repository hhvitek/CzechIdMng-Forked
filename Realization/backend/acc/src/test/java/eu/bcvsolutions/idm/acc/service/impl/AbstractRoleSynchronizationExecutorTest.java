 ...
 *
 */
public abstract class AbstractRoleSynchronizationExecutorTest<T extends Codeable> extends AbstractBulkActionTest {
	...
	public void testSyncRolesAssignToUsers() {
		AbstractSysSyncConfigDto syncConfigCustom = createSyncConfig();
		SysSystemDto targetSystem = helper.createTestResourceSystem(true);

		SysSystemMappingDto targetMapping = getTargetMapping(targetSystem);
		SysSystemAttributeMappingDto membershipAttribute = prepareAttributeMappings(targetMapping);
		SysSyncIdentityConfigDto targetSyncConfig = createTargetSyncConfig(targetSystem);
		...
		schemaUserAttributeFilter.setSystemId(targetSystem.getId());
		...
		roleConfigDto.setMembershipSwitch(true);
		roleConfigDto.setMemberSystemMapping(targetMapping.getId());
		roleConfigDto.setMemberOfAttribute(membershipAttribute.getId());
		...
		// Start sync of targets
		helper.startSynchronization(targetSyncConfig);
		helper.checkSyncLog(targetSyncConfig, SynchronizationActionType.CREATE_ENTITY, 2, OperationResultType.SUCCESS);
		T targetOne = getTargetByCode(usernameOne);
		Assert.assertNotNull(targetOne);
		T targetTwo = getTargetByCode(usernameTwo);
		Assert.assertNotNull(targetTwo);
		...
		roleAccountDtos.forEach(roleAccountDto -> {
			Assert.assertEquals(1, getAssignmentsForRoleAndTarget(roleAccountDto.getRole(), targetOne).size());
			Assert.assertEquals(1, getAssignmentsForRoleAndTarget(roleAccountDto.getRole(), targetTwo).size());
		});
		
		this.getBean().cleanAfterTest(syncConfigCustom, roleSystemId, log, roleAccountDtos);
	}

	protected abstract SysSystemAttributeMappingDto prepareAttributeMappings(SysSystemMappingDto targetMapping);

	protected abstract List<AbstractRoleAssignmentDto> getAssignmentsForRoleAndTarget(UUID roleId, T identityOne);

	protected abstract T getTargetByCode(String code);

	protected abstract SysSystemMappingDto getTargetMapping(SysSystemDto userSystem);
	...
	public void testSyncRolesDeleteAssignedFromUsers() {
		...
		// Start sync of users
		helper.startSynchronization(targetSyncConfig);
		helper.checkSyncLog(targetSyncConfig, SynchronizationActionType.CREATE_ENTITY, 3, OperationResultType.SUCCESS);
		T identityOne = getTargetByCode(usernameOne);
		Assert.assertNotNull(identityOne);
		T identityTwo = getTargetByCode(usernameTwo);
		Assert.assertNotNull(identityTwo);
		T identityThree = getTargetByCode(usernameThree);
		...
		// Every role should be assigned to userOne and userTwo.
		roleAccountDtos.forEach(roleAccountDto -> {
			Assert.assertEquals(1, getAssignmentsForRoleAndTarget(roleAccountDto.getRole(), identityOne).size());
			Assert.assertEquals(1, getAssignmentsForRoleAndTarget(roleAccountDto.getRole(), identityTwo).size());
			Assert.assertEquals(0, getAssignmentsForRoleAndTarget(roleAccountDto.getRole(), identityThree).size());

			// Assign role to identityThree.
			assignRoleToTarget(identityThree, roleAccountDto.getRole());
		});
		...
		roleAccountDtos.forEach(roleAccountDto -> {
			Assert.assertEquals(1, getAssignmentsForRoleAndTarget(roleAccountDto.getRole(), identityOne).size());
			Assert.assertEquals(1, getAssignmentsForRoleAndTarget(roleAccountDto.getRole(), identityTwo).size());
			Assert.assertEquals(1, getAssignmentsForRoleAndTarget(roleAccountDto.getRole(), identityThree).size());
		});
	...
	protected abstract void assignRoleToTarget(T target, UUID role);
	...
	public AbstractSysSyncConfigDto createSyncConfig() {
		...
		List<SysSystemAttributeMappingDto> attributes = schemaAttributeMappingService.find(attributeMappingFilter, null)
				.getContent();
		SysSystemAttributeMappingDto uidAttribute = attributes.stream().filter(SysSystemAttributeMappingDto::isUid).findFirst().orElse(null);
	...
	public SysSyncIdentityConfigDto createTargetSyncConfig(SysSystemDto system) {
		SysSystemMappingFilter mappingFilter = new SysSystemMappingFilter();
		mappingFilter.setEntityType(getEntityType());
		mappingFilter.setSystemId(system.getId());
		mappingFilter.setOperationType(SystemOperationType.SYNCHRONIZATION);
		List<SysSystemMappingDto> mappings = systemMappingService.find(mappingFilter, null).getContent();
		Assert.assertEquals(1, mappings.size());
	...
	private AbstractRoleSynchronizationExecutorTest getBean() {
